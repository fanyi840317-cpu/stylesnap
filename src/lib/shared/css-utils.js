/**
 * css-utils.js — Shared CSS-to-Tailwind utilities (Plain JavaScript)
 * Used by exportWorker.ts (Web Worker)
 *
 * This file contains the COMPLETE mapping logic from tailwind-mapper.ts
 * to ensure consistent behavior between main thread and worker.
 */
...
function spacingClass(prefix, val) {
  ...
  if (pct) {
    ...
    const fractions = {
      ...
      60: '3/5', 80: '4/5', 100: 'full',
    }
    for (const [k, v] of Object.entries(fractions)) {
      if (Math.abs(n - parseFloat(k)) < 0.1) return `${prefix}-${v}`
    }
  }
...
const DISPLAY_MAP = {
  ...
  'inline-grid': 'inline-grid', 'table': 'table', 'table-cell': 'table-cell',
  'table-row': 'table-row', 'none': 'hidden', 'contents': 'contents',
  'list-item': 'list-item',
}
...
const OVERFLOW_MAP = {
  ...
  visible: 'overflow-visible', scroll: 'overflow-scroll', clip: 'overflow-clip',
}
...
const ALIGN_ITEMS_MAP = {
  flex_start: 'items-start', 'flex-start': 'items-start',
  flex_end: 'items-end', 'flex-end': 'items-end',
  center: 'items-center', baseline: 'items-baseline', stretch: 'items-stretch',
}

const JUSTIFY_CONTENT_MAP = {
  flex_start: 'justify-start', 'flex-start': 'justify-start',
  flex_end: 'justify-end', 'flex-end': 'justify-end',
...
const OPACITY_MAP = {
  ...
  '0.15': 'opacity-15', '0.2': 'opacity-20', '0.25': 'opacity-25',
  '0.3': 'opacity-30', '0.35': 'opacity-35', '0.4': 'opacity-40',
  '0.45': 'opacity-45', '0.5': 'opacity-50', '0.6': 'opacity-60',
  '0.7': 'opacity-70', '0.75': 'opacity-75', '0.8': 'opacity-80',
  '0.9': 'opacity-90', '0.95': 'opacity-95', '1': 'opacity-100',
}

// Main property → Tailwind mappers
const PROPERTY_MAPPERS = {
  // Layout
  ...
  // Border
  'border-width': (v) => {
    const bw = { '0px': 'border-0', '1px': 'border', '2px': 'border-2', '4px': 'border-4', '8px': 'border-8' }
  ...
  // Z-index
  'z-index': (v) => {
    const zi = { '0': 'z-0', '10': 'z-10', '20': 'z-20', '30': 'z-30', '40': 'z-40', '50': 'z-50', 'auto': 'z-auto' }
    return zi[v] || `z-[${v}]`
  },
...
export function generateReactComponent(html, css, opts = {}) {
  const { classes, matchRate } = cssToTailwind(css)
  const jsx = htmlToJSX(html)
  const clsStr = classes.length > 0 ? ` className="${classes.join(' ')}"` : ''
  const matchInfo = opts.includeMatchRate ? `  {/* Tailwind match: ${Math.round(matchRate * 100)}% */}` : ''
...
export function generateVueComponent(html, css, opts = {}) {
  const { classes, matchRate } = cssToTailwind(css)
...
// ─── Helper: build full component export (used by exportWorker) ──
export function buildComponentExport(format, html, css, componentName) {
  switch (format) {
    case 'react':
      return generateReactComponent(html, css, { includeMatchRate: true })
    case 'vue':
      return generateVueComponent(html, css, { includeMatchRate: true })
    case 'tailwind': {
      const { classes, unmatched } = cssToTailwind(css)
      const lines = [
        '/* Tailwind Classes */',
        ...classes.map(c => `.${c} { /* ${c} */ }`),
        '',
        '/* Unmatched CSS (add manually) */',
        ...Object.entries(unmatched).map(([k, v]) => `${k}: ${v};`),
      ]
      return lines.join('\n')
    }
    case 'css-module': {
      const { classes, unmatched } = cssToTailwind(css)
      const lines = [
        '/* CSS Module with Tailwind */',
        ...classes.map(c => `.${c} { /* mapped to ${c} */ }`),
        '',
        '/* Unmatched CSS */',
        ...Object.entries(unmatched).map(([k, v]) => `.unmatched-${k.replace(/[^a-z0-9]/gi, '-')} { ${k}: ${v}; }`),
      ]
      return lines.join('\n')
    }
    default:
